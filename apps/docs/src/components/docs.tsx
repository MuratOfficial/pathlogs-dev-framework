...
/** Заголовок раздела. `id` служит якорем и попадает в оглавление справа. */
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-edge bg-surface-2/40 text-left">
              <th className="px-4 py-2.5 font-medium text-muted">Проп</th>
              <th className="px-4 py-2.5 font-medium text-muted">Тип</th>
              <th className="px-4 py-2.5 font-medium text-muted">По умолчанию</th>
              <th className="px-4 py-2.5 font-medium text-muted">Описание</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              <tr key={row.name} className="border-b border-edge/60 align-top last:border-0">
                <td className="whitespace-nowrap px-4 py-3">
                  ...
                  {row.required && (
                    <span className="ml-1 text-danger" title="обязательный">
                      *
                    </span>
                  )}
                </td>
...
const CALLOUT: Record<CalloutTone, { label: string; className: string; icon: string }> = {
  note: {
    label: "Заметка",
...
export function Callout({
  ...
  return (
    <aside className={`not-prose my-5 flex gap-3 rounded-xl border p-4 ${t.className}`}>
      <svg
      ...
      <div className="min-w-0 text-sm leading-relaxed text-foreground/85">
        <p className="mb-1 font-semibold text-foreground">{title ?? t.label}</p>
        {children}
      </div>
    </aside>
  );
