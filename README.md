...
### Выпуск новой версии в npm

Пакеты выпускаются **вместе, одной версией**: они ссылаются друг на друга
точной версией, и разъехавшиеся номера почти всегда означают сломанную
установку у пользователя.
...
1. **`npm version`** поднимает номер во всех пакетах монорепо.
2. **`sync-versions`** приводит внутренние ссылки к новым номерам. Это
   обязательный шаг: `npm version` их не трогает, и без него `core@0.2.0`
   уедет в реестр с зависимостью `hooks@0.1.0` — а код, которого там ещё
   нет, упадёт уже в чужом проекте.
3. **`npm install`** пересобирает связи в `node_modules`.
4. **`npm run release`** прогоняет проверку (типы, тесты, согласованность
   версий) и публикует в порядке зависимостей: tokens → hooks → core → ui.

Посмотреть, что уедет, ничего не публикуя: `npm run release:dry`.

> **Если после `npm version` установка вела себя странно.** Пока ссылки
> указывают на старую версию, npm считает её обычной внешней зависимостью
> и скачивает опубликованный пакет во вложенный `node_modules` внутри
> рабочей копии. Такая копия перекрывает рабочую, и сборка падает на
> «нет такого экспорта» у кода, который вот он, рядом. Лечится удалением
> вложенных `node_modules` и повторной установкой — либо просто не
> пропускайте шаг `sync-versions`.

Что и когда обновлять:

| Изменилось | Кого публиковать |
|---|---|
| `packages/*` | соответствующий пакет и всё, что от него зависит |
| `registry/widgets/*` | `@toimetdev/pathlogs-ui` — реестр едет внутри CLI |
| `apps/docs` | ничего, сайт разворачивается отдельно |

Реестр попадает в пакет CLI на этапе упаковки (`prepack`), а сразу после
неё копия убирается (`postpack`) — иначе в монорепо она перекрывала бы
настоящий каталог `registry/`, и `npm run cli list` показывал бы состояние
на момент последней сборки.
