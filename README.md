...
`bump` принимает и точную версию (`npm run bump 0.3.0`), и шаг
(`patch`, `minor`, `major`). Он переписывает номера и внутренние ссылки
одним проходом, затем ставит зависимости. `release` прогоняет проверку
(типы, тесты, согласованность версий) и публикует в порядке зависимостей:
tokens → hooks → core → ui.
...
> ещё нет, и установка падает с `ETARGET`. Когда версия в реестре есть,
> получается хуже: npm скачивает опубликованный пакет во вложенный
> `node_modules`, тот перекрывает рабочую копию, и сборка ломается на
> «нет такого экспорта» у кода, который лежит рядом. Если это уже случилось —
> удалите `node_modules` внутри `packages/*` и `apps/*` и переустановите.
